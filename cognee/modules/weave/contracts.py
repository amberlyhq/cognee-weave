...
class RecallRequest(BaseModel):
    """Selectors accepted from Amberly; storage routing stays internal."""

    model_config = ConfigDict(extra="forbid")

    mode: RecallMode = "repository_context"
    query: str = Field(min_length=1)
    github_repository_ids: list[int] = Field(default_factory=list)
...
class RecallResponse(BaseModel):
    ...
    graph_candidates: list[RecallCandidate] = Field(default_factory=list)
    vector_candidates: list[RecallCandidate] = Field(default_factory=list)
    diagnostics: list[RecallDiagnostic] = Field(default_factory=list)
    native_memory: str | None = Field(default=None)
...
class SurfaceResponse(BaseModel):
    """Export/visualization data from one resolved organization scope."""
    ...
    repositories: list[RepositoryReference] = Field(default_factory=list)
    nodes: list[RecallCandidate] = Field(default_factory=list)
    edges: list[SurfaceEdge] = Field(default_factory=list)
    native_graph: str | None = Field(default=None)
...
class ReviewSession(BaseModel):
    ...
    steps: list[ReviewSessionStep] = Field(default_factory=list)
...
class MergeKnowledgeChange(BaseModel):
    ...
    complete: bool
    review_context: str = Field(default="")
